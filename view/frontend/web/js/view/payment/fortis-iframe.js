(function() {
    function generateIFrame() {
        ...
        if (!fortisDataElement) {
            console.log('fortis-data element not found');
            return;
        }
        const config = JSON.parse(fortisDataElement.getAttribute('data-config'));
        if (!config || !config.main_options) {
            console.log('fortis-data config missing');
            return;
        }

        var moduleName = config.main_options.environment === 'production'
            ? 'fortis-commerce-prod'
        ...
        function buildCancelButton(config, isCheckout)
        {
            ...
            require(['mage/url'], function(_urlBuilder) {
    ...
    window.fortisGenerateIFrame = generateIFrame;

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() {
            if (document.getElementById('fortis-data')) {
                generateIFrame();
            }
        });
    } else {
